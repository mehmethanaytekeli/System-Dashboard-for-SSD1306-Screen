...
while True:
  ...
  # Ağ hızı bilgisi
    ...
    draw.text((x, y + 10), f"CPU: {round(cpu_temp)}°C", font=font, fill=255)  # Değişen satır
    draw.text((x, y + 20), f"Usage: {cpu_usage}%", font=font, fill=255)
    draw.text((x, y + 30), f"IP: {ip_address}", font=font, fill=255)
    draw.text((x, y + 40), f"RAM: {ram_usage}%", font=font, fill=255)

    # Ok çizimleri
    arrow_y = y + 52
    draw.polygon([(0, arrow_y+5), (4, arrow_y+5), (2, arrow_y+2)], fill=255)  # ▼
    draw.polygon([(64, arrow_y+2), (68, arrow_y+2), (66, arrow_y+5)], fill=255)  # ▲
    draw.text((6, arrow_y-2), format_speed(upload_speed), font=font, fill=255)
    draw.text((70, arrow_y-2), format_speed(download_speed), font=font, fill=255)
